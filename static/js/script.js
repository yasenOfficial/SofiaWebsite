...
const landmarks = [
  ...
  },
  {
    ...
    function updateProgress() {
        const pct = (visited.size / landmarks.length) * 100;
        progressFill.style.width = `${pct}%`;
        landmarkCounter.textContent = `${visited.size} / ${landmarks.length}`;
    }
...
toggleStreetViewBtn.onclick = () => {
    if (currentIndex < 0) return; // no landmark selected

    const lm = landmarks[currentIndex];

    // load the Google Maps embed URL for this landmark
    sv.src = lm.embed;

    const isVisible = !sv.classList.contains("d-none");

    // toggle visibility
    sv.classList.toggle("d-none");
    carouselContainer.classList.toggle("d-none"); // hide carousel when street view is open

    // update button text
    toggleStreetViewBtn.innerHTML = isVisible
        ? `<i class="fas fa-street-view me-1"></i>View Street`
        : `<i class="fas fa-street-view me-1"></i>Close Street`;
};
